# GEMINI.md — Indian Motor Accident Claims Drafting (Gemini CLI port)
...
**Plugin:** `indian-mact-drafting` · **Forum:** Motor Accident Claims Tribunal (territorial district) / High Court for Section 173 MV Act appeal / Magistrate Court for rash and negligent driving · **Publisher:** Wolfgang Rush · **License:** MIT

---

## 🇮🇳 WHO YOU ARE TALKING TO

The user is an **enrolled Indian advocate** drafting MACT and motor-vehicle litigation pleadings for filing. They are running the 6-agent drafting pipeline (reader → format → drafter → verifier → refiner → overseer) and expect filing-grade output (.docx) with anti-hallucination discipline.
...
Every pipeline run reads the user's `case-config.md + state-config.md` from the case folder. Substitute every `{{config.X}}` placeholder in the format-shell with the real value before passing to the Drafter.
...
- Preserve the advocate's bench's idiom (substituted from case-config.md + state-config.md)

### Rule 8 — STOP on missing inputs
- Reader STOPS if required law PDF missing
- Format STOPS if case-config.md + state-config.md missing or incomplete
...
```
<case-folder>/
├── case-config.md + state-config.md    ← required, advocate-supplied
...
Section 166 MV Act claim · Section 163A no-fault liability · Section 173 appeal · Hit-and-run claim · Insurance Company impleadment · Section 304A IPC / Section 106 BNS criminal complaint · Solatium Fund claim

Each case type has its own template in `skills/ (MACT case-type templates)`.
